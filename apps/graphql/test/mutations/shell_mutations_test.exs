defmodule GraphQl.ShellMutationsTest do
  ...
  describe "updateShell" do
    test "it will update an existing shell instance" do
      %{email: e} = user = insert(:user, roles: %{admin: true})

      expect(Pods, :fetch, fn _ -> {:ok, Pods.pod("plrl-shell-2", e)} end)
      expect(Core.Shell.Scm, :setup_repository, fn
        :github, ^e, "tok", nil, "demo" ->
          {:ok, "git@github.com:pluralsh/demo.git", "pub-key", "priv-key", nil}
      end)
      ...
      {:ok, %{data: %{"updateShell" => updated}}} = run_query("""
        mutation Update($attrs: CloudShellAttributes!) {
          updateShell(attributes: $attrs) {
            id
            updatedAt
          }
        }
      ...
      assert updated["id"]
      assert created["updatedAt"] != updated["updatedAt"]
    end
  end
