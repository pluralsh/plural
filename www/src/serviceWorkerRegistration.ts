...
const checkServiceWorker = (): Promise<boolean | void> => fetch(serviceWorkerURL, { headers: { 'Service-Worker': 'script' } }).then(response => {
...
const onServiceWorkerStateChange = (registration: ServiceWorkerRegistration, config: Config) => () => {
  const { installing } = registration
