defmodule Core.Services.Payments do
  ...
  def remove_pro(%Account{} = account) do
    case Core.Repo.preload(account, [:root_user, subscription: :plan], force: true) do
      %Account{subscription: %PlatformSubscription{plan: %PlatformPlan{enterprise: false}} = sub} ->
        cancel_platform_subscription(sub, sub.root_user)
      %Account{} -> {:ok, nil}
    end
  end
