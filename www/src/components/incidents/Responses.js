...
import { IncidentSort, Order, IncidentStatus, IncidentFilter } from './types'
...
function StatusSelector({statuses, setStatuses}) {
  ...
  return (
    <>
    <Box direction='row' align='center' gap='xsmall' ref={ref} flex={false} 
         onClick={() => setOpen(true)} pad={{horizontal: 'small', vertical: 'xsmall'}}>
    ...
    {open && (
      <Drop target={ref.current} align={{top: 'bottom'}} 
        ...
        <Box flex={false} gap='xsmall' pad='xsmall'>
          {Object.values(IncidentStatus).map((status) => (
            <Box key={status} direction='row' gap='small' align='center' round='xsmall'
                 hoverIndicator='tone-light' onClick={() => onClick(status)} pad='xsmall'>
              <Box flex={false} width='10px'>
              {hasStatus.has(status) && <Checkmark size='small' color='brand' />}
              </Box>
              <Text size='small' weight={500}>{status}</Text>
            </Box>
          ))}
        </Box>
      </Drop>
  ...
  return (
    <Box fill>
      ...
      <IncidentToolbar>
        ...
        <StatusSelector statuses={statuses} setStatuses={setStatuses} />
      </IncidentToolbar>
      <Box fill>
