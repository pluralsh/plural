...
import {
  ArrowTopRightIcon, Button, Chip, PageCard, PageTitle, Sidecar, SidecarItem,
...
export function Clusters(): ReactElement | null {
  ...
  return (
    <Flex
      ...
      paddingBottom={16}
    >
      <SidenavLayout queue={queue} />
      <ContentLayout queue={queue} />
      <SidecarLayout queue={queue} />
    </Flex>
  )
}
...
function ContentLayout({ queue }: { queue: Queue }): ReactElement | null {
  ...
  return (
    ...
    >
      ...
      <UpgradesList
        upgrades={data.upgradeQueue?.upgrades?.edges}
        acked={queue.acked}
      />
    </Flex>
  )
...
function SidecarLayout({ queue }: { queue: Queue }): ReactElement {
  return (
    <>
      <Flex
        maxWidth={sidecarWidth}
        minWidth={sidecarWidth}
        gap={24}
        direction="column"
        paddingTop={68}
      >
