...
import {
  CREATE_QUICK_STACK_MUTATION,
  INSTALL_STACK_SHELL_MUTATION,
} from '../../../queries'
...
const toDefaultSteps = (
...
): Array<WizardStepConfig> => [
  {
    ...
    Icon: AppsIcon,
    node: (
      <WizardPicker items={toPickerItems(applications, provider, forcedApps)} />
    ),
    isDefault: true,
  },
  {
...
const buildSteps = async (
  ...
  for (const app of selectedApplications) {
    ...
    const { node: recipeBase } = edges?.find(
...
const install = async (
  ...
  const toDataTypeValues = (context, datatype) =>
    Object.keys(context || {}).reduce(
      (acc: Array<any>, key) =>
        context[key].type === datatype
  ...
  const configuration = apps.reduce(
    (acc, app) => ({
      ...acc,
      [app.label!]: toAPIContext(app.data?.context || {}),
    }),
    {}
  )
  const domains = apps.reduce(
    (acc: Array<any>, app) => [
      ...acc,
      ...toDataTypeValues(app.data?.context || {}, Datatype.Domain),
    ],
    []
  )
  const buckets = apps.reduce(
    (acc: Array<any>, app) => [
      ...acc,
      ...toDataTypeValues(app.data?.context || {}, Datatype.Bucket),
    ],
    []
  )
...
export { toDependencySteps, toDefaultSteps, buildSteps, toPickerItems, install }
