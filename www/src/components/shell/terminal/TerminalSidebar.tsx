...
function TerminalSidebar({ shell, showCheatsheet, ...props }) {
  ...
  const { workingSteps, skipConsoleInstall } = useMemo(() => {
    ...
    if (console) return { workingSteps, skipConsoleInstall: false }
  ...
  function markDemoAsComplete() {
    mutation()
    setTerminalOnboardingSidebar(false)
  }
    ...
    <>
      ...
      >
        <Flex
          ...
          direction="column"
        >
          {fresh || terminalOnboardingSidebar ? renderDemo() : showCheatsheet ? renderCheatsheet() : null}
        </Flex>
      </Div>
      {isModalOpen && (
        <Fireworks
