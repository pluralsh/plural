...
function PriceLine({ count, price, yearly, name, top }: LinePropsType) {
  return (
    <Flex
      align="center"
      gap="medium"
...
function BillingPreview({
  ...
  const { proPlatformPlan, proYearlyPlatformPlan, annualDiscount } =
  ...
  const currCluster = useMemo(
    () =>
      account?.subscription?.plan
        ? getPrice(account.subscription?.plan, 'CLUSTER')
        : 0,
    [account]
  )
  const currUser = useMemo(
    () =>
      account?.subscription?.plan
        ? getPrice(account.subscription?.plan, 'USER')
        : 0,
    [account]
  )
  const pClusters = useMemo(
    () =>
      yearly
        ? getPrice(proYearlyPlatformPlan, 'CLUSTER')
        : getPrice(proPlatformPlan, 'CLUSTER'),
    [yearly, proYearlyPlatformPlan, proPlatformPlan]
  )
  const pUsers = useMemo(
    () =>
      yearly
        ? getPrice(proYearlyPlatformPlan, 'USER')
        : getPrice(proPlatformPlan, 'USER'),
    [yearly, proYearlyPlatformPlan, proPlatformPlan]
  )
  const clusterPrice =
    discountPreview || isProfessional ? pClusters : currCluster
