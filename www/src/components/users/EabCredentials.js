...
function EabCredentialHeader() {
  return (
    <TableRow>
      <HeaderItem text='Key Id' width='20%' />
      <HeaderItem text='Hmac Key' width='30%' />
...
function EabCredential({eab}) {
  return (
    <TableRow>
      <HeaderItem text={eab.keyId} nobold truncate width='20%' />
      <HeaderItem text={obscure(eab.hmacKey)} nobold width='30%' />
      <Box width='20%' direction='row' gap='xsmall' align='center'>
        <Provider provider={eab.provider} width='25px' />
        <Text size='small' weight={500}>{eab.cluster}</Text>
      </Box>
  ...
  return (
    <Box fill>
      ...
      {data.eabCredentials.map((eab) => (
        <EabCredential key={eab.id} eab={eab} />
      ))}
    </Box>
  )
}